...
namespace Voilaah\MallApi\Behaviors;
 ...
 *
 *   public $implement = ['Voilaah.MallApi.Behaviors.RestController'];
 *
 * @author Chris
 */
class RestController extends ControllerBehavior
{
    ...
    use \Voilaah\MallApi\Classes\Traits\RestTrait;
    ...
    public function index()
    {
        ...
        $order = isset($options['sortOrder']) ? $this->getSortOrder($options['sortOrder']) : $this->getSortOrder();
        $this->sortKey      = $order->key();
        $this->sort         = $order->property();
        $this->direction    = Request::input('direction', $order->direction());
        /*
         * Default options
         */
        extract(array_merge([
    ...
    public function findModelObject($recordId)
    {
         ...
         */
        if (method_exists($model, 'bySlugOrId')) {
            $id =  (is_numeric($recordId)) ? $recordId : ':slug';
    ...
    protected function getSortOrder($sortOrder = null): SortOrder
    {
